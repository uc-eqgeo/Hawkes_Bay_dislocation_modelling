...
### USER: define which model names you want to make figures for
...
def make_def_cross_section(extension_name, ax):
    """ makes a cross section figure of deformation through the gridded vertical def data
    ax tells it which axis to plot on in the larger subplot figure """

    #load data
    tiled_fault_dict = pkl.load(open(f"tiled_fault_dictionaries/tiled_fault_dict_{extension_name}.pkl", 'rb'))
    preferred_slip = np.load(f"out_data/preferred_slip_array_{extension_name}.npy")
    ...
    x_array, y_array = tiled_fault_dict['gf_grid_x'], tiled_fault_dict['gf_grid_y']
    ...
    above_zero = ~below_zero    # takes the opposite of the boolean array
    ...
    x_above_zero, y_above_zero = distances_km[above_zero], cross_sect_values[above_zero]
    ...
    # make it pretty
    ax.set(xlim=(0., distances_km.max()), ylim=(-1.5, 3.25))
    ...
    ax.text(1, 0.2, f'A.L. disp.:{ahuriri_subsidence:.2f}', fontsize=6,
            color=(5 / 255, 113 / 255, 176 / 255, 1))

    #plt.savefig(f"figures/{extension_name}/vert_def_cross_section_{extension_name}.png")

def make_slip_cross_section(extension_name, ax):
    #for extension_name in extension_list:
    #load data
    tiled_fault_dict = pkl.load(open(f"tiled_fault_dictionaries/tiled_fault_dict_{extension_name}.pkl", 'rb'))
    tiled_fault = tiled_fault_dict['tiled_fault']
    preferred_slip = np.load(f"out_data/preferred_slip_array_{extension_name}.npy")
    # load elevation profile csv file and interface geometry cross-section pts
    ...
    # same thing but make it denser for slip values
    ...
    cross_sect_slip_values = griddata(patch_centroids_xy, preferred_slip, line_xy_dense,
    ...
    # split slip values into bins
    # unique_slip_values = np.unique(cross_sect_slip_values)
...
#   # subset cross section x and y by + and - y value
    ...
    # labels for cross-section ends, Ahuriri and Cape Kidnappers, V.E. ticks
...
def make_figure(extension_list):
    """makes a 3-part plot with the fault patches and slip, and vertical deformation, and cross-section"""
    ...
    for extension_name in extension_list:
        ...
        gf_grid = tiled_fault_dict['gf_grid']
        xgrid = tiled_fault_dict['gf_grid_x']
        ygrid = tiled_fault_dict['gf_grid_y']
        patch_size = tiled_fault_dict['fault_patch_size']
        vertical_disp_grid = np.sum(np.array([preferred_slip[i] * gf_grid[i] for i in range(len(preferred_slip))]),
                                  axis=0)
        ...
        patches = tiled_fault.patch_collection2d(cmap=custom_slip_colormap, patch_alpha=1, line_colour="none",
                                                 line_width=0.5)
        patches.set_array(preferred_slip)
        ...
        Mw, avg_slip = get_rect_Mw(preferred_slip=preferred_slip, patch_edge_size=patch_size)
        ...
        #plot fault mapping and model fault outline on left plot
        ...
        make_def_cross_section(extension_name, ax=axs[2])
        make_slip_cross_section(extension_name, ax=axs[3])

        fig.suptitle(f"Model {extension_name}", fontsize=8)
        fig.tight_layout()

        #save figure
        if not os.path.exists(f"figures"):
            os.mkdir(f"figures")
        fig.savefig(f"figures/fig_{extension_name}.png", transparent=True,dpi=300)
        #fig.savefig(f"figures/fig_{extension_name}.pdf", transparent=True, dpi=300)


make_figure(extension_list)